...
import {
  TAG_DESIGN_PRESETS,
  applyShapeAdjustments,
} from "../../utils/tagDesignPresets";
...
const TagPreview = ({
  ...
  // Calculate dimensions early to avoid temporal dead zone in useEffect
  ...
  const handleMouseDown = (e) => {
    if (orderType !== "database" || currentSide !== qrCodeSide || !qrCodeValue)
      return;
    setIsDragging(true);
  ...
  const handleMouseMove = (e) => {
    ...
    const newX = Math.max(
      10,
      Math.min(90, ((e.clientX - rect.left - dragStart.x) / rect.width) * 100)
    );
    const newY = Math.max(
      10,
      Math.min(90, ((e.clientY - rect.top - dragStart.y) / rect.height) * 100)
    );
  ...
  const toggleSide = () => {
    ...
    fetch("http://127.0.0.1:7242/ingest/9465c166-d076-4973-a708-7b037fdbb184", {
      ...
      body: JSON.stringify({
        location: "TagPreview.js:141",
        message: "toggleSide called",
        data: { currentSide: currentSide, isFlipping: isFlipping },
        timestamp: Date.now(),
  ...
  const getFontStyle = (fontId) => {
    ...
    if (fontId || selectedFont) {
      ...
      if (font) {
        return {
          ...
          fontWeight:
            font.style === "Bold" || font.style === "Statement"
              ? "700"
              : font.style === "Modern"
              ? "600"
              : "400",
    ...
    if (selectedDesign) {
      const appliedDesign = applyShapeAdjustments(
        selectedDesign,
        normalizedShape
      );
      return {
        ...
        {line1 && (
          <div
            ...
            style={{
              ...
              textShadow:
                "0 2px 4px rgba(0,0,0,0.2), 0 0 2px rgba(255,255,255,0.5)",
              lineHeight: "1.2",
            }}
          >
  ...
  return (
    <div
      className={`bg-white dark:bg-dark-surface rounded-xl shadow-lg dark:shadow-card-dark p-4 border border-light-border dark:border-dark-border ${className}`}
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={(e) => {
              // #region agent log
              fetch(
                "http://127.0.0.1:7242/ingest/9465c166-d076-4973-a708-7b037fdbb184",
                {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    location: "TagPreview.js:474",
                    message: "TagPreview flip button clicked",
                    data: { currentSide: currentSide, isFlipping: isFlipping },
                    timestamp: Date.now(),
                    sessionId: "debug-session",
                    hypothesisId: "F",
        ...
        >
          <div
            ...
            style={{
              ...
              transform:
                currentSide === 2 ? "rotateY(180deg)" : "rotateY(0deg)",
            }}
          >
            {/* Side 1 - Front */}
            <div
      ...
      <div className="flex items-center justify-center gap-4 mt-3">
        ...
        >
          Side 1:{" "}
          {orderType === "database" && qrCodeSide === 1 ? "QR Code" : "Text"}
        </div>
        ...
        >
          Side 2:{" "}
          {orderType === "database" && qrCodeSide === 2 ? "QR Code" : "Text"}
        </div>
      </div>
      {selectedDesign && (
        <div className="mt-2 text-center">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Style: <strong>{selectedDesign.name}</strong> •{" "}
      ...
      <p className="text-xs text-gray-600 dark:text-gray-400 mt-2 text-center">
        {orderType === "database"
          ? `QR code will appear on Side ${qrCodeSide}. Use the flip button in the form to see both sides.`
          : "Use the flip button in the form to see both sides"}
      </p>
    </div>
  );
};
